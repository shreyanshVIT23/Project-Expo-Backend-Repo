...
def excel_to_sqlite(excel_file, db_file):
    ...
    required_columns = ["Room No.", "Cabin No.",
                        "Name", "Mobile No.", "Status"]
    if not all(col in df .columns for col in required_columns):
        print(
            f"Missing required columns. Ensure the file has: {required_columns}")
        return
    ...
    try:
        for _, row in df .iterrows():
            ...
            if not transformed_room_no:
                print(
                    f"Skipping invalid room number format: {original_room_no}")
                continue
            ...
            VALUES (?, ?, ?, ?)
            """, (row['name'], row['cabin_no'], transformed_room_no, row['phone_number']))
