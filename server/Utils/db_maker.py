...
def insert_descriptions_from_connections(
    ...
    unique_coordinates = [coord for coord,
                          count in coordinate_count .items()if count == 1]
    ...
    for anchor_id, x, y in unique_point_ids:
        cursor .execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (anchor_id, f"Anchor at ({x}, {y})",
             "Unique anchor point", "Auto-generated description"),
        )
...
def insert_connections(connections, cursor):
    ...
    print(f"{len(connections) - len(skipped_connections)} connections inserted successfully into the connections table.")
    ...
    for point_id, x, y in anchor_points:
        ...
        if closest_text in {'T', 'B', 'G'}:
            ...
            for text, x1, y1 in svg_data:
                if dbs(text):
                    distance = calculate_distance(x1, y1, x, y)
                    if distance < closest_numeric_distance and distance > closest_distance:
        ...
        if "Lift" in closest_text or "Stair" in closest_text:
            ...
            nearby_texts = sorted(
                svg_data,
                key=lambda t: calculate_distance(t[1], t[2], x, y)
            )[:8]

            valid_texts = [text for text, _,
                           _ in nearby_texts if text .isdigit() and dbs(text)]
        ...
        if closest_text is not None:
            ...
            """, (closest_text, f"{extra_info} | {lift_stair_info}"if lift_stair_info else extra_info, point_id))
